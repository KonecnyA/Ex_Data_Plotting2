...
Baltimore.NEI  <- subset(NEI, fips == "24510" & type == "ON-ROAD", c("Emissions", "year", "fips"))
LosAngeles.NEI <- subset(NEI, fips == "06037" & type == "ON-ROAD", c("Emissions", "year", "fips"))
...
LosAngeles.total.PM25.year <- aggregate(Emissions ~ year, data = LosAngeles.NEI, sum)
...
##ggplot() + 
##geom_line(aes(year, Emissions, City = "steelblue"), Baltimore.total.PM25.year) +  
##geom_line(aes(year, Emissions, City = "pink"), LosAngeles.total.PM25.year) +
...
qplot(year,
  ...
  geom_line(aes(colour = fips),
            linetype = 1,
            size = 1.5) +ggtitle(expression("Baltimore City vs Los Angeles County " ~ PM[2.5] ~ " by Year for Motor Vehicle Sources")) +
